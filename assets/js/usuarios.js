...
$(document).ready(function() {
    ...
    const reporteTitulo = "REPORTE DE USUARIOS";
    ...
    $("#formUsuario").on("submit", function(e) {
        ...
        // Si es nuevo o se quiere cambiar contraseña
        if (id === "" || pass !== "") {
            if (pass.length < 8) {
                showNotification('La contraseña debe tener como mínimo 8 caracteres', 'error');
                return false;
            }
            if (pass !== confirm) {
        ...
        const urlDestino = (id === "") 
                ? "../../modules/usuarios/usuarios_guardar.php" 
                : "../../modules/usuarios/usuarios_actualizar.php";
    ...
    $('#tblUsuarios').on('click', '.btnEditar', function() {
        ...
        $("#modalTitulo").html('<i class="bi bi-pencil-square me-2" style="color: var(--color-yellow);"></i>Actualizar Usuario: ' + d.username);
        $("#passHelp").text("Dejar en blanco para mantener la contraseña actual");
...
function prepararNuevo() {
    $("#formUsuario")[0].reset();
    $("#id_usuario").val("");
    $("#modalTitulo").html('<i class="bi bi-person-plus-fill me-2" style="color: var(--color-yellow);"></i>Registrar Nuevo Usuario');
    $("#passHelp");
...
function eliminarUsuario(btn) {
    ...
    $("#usuarioEliminarId").val(datos.id_usuario);
    $("#eliminarNombre").text(datos.nombre);
    $("#eliminarApellido").text(datos.apellido);
    $("#eliminarUsername").text(datos.username);
    $("#eliminarEmail").text(datos.email || 'No especificado');
    
    // Obtener y mostrar el nombre del rol
    const idRol = datos.id_rol;
    const nombreRol = obtenerNombreRol(idRol);
    $("#eliminarRol").text(nombreRol);
...
function obtenerNombreRol(idRol) {
    // Mapeo de roles según tu sistema
    const roles = {
        '1': 'Administrador',
        '2': 'Vendedor',
        '3': 'Almacenero',
    ...
    return roles[idRol] || 'Rol no definido (ID: ' + idRol + ')';
}

function confirmarEliminarUsuario() {
    const id = $("#usuarioEliminarId").val();
    ...
    if (typeof alertify !== 'undefined') {
        alertify.confirm(
            ...
            `<div style="text-align: center;">
                ...
                <p class="text-muted">Esta acción eliminará permanentemente al usuario y no podrá ser recuperado.</p>
            </div>`,
            function() {
                ejecutarEliminacionUsuario(id);
            },
            function() {
...
function ejecutarEliminacionUsuario(id) {
    // Mostrar loading
    showNotification('Eliminando usuario...', 'info');
    
    $.ajax({
        url: "../../modules/usuarios/usuarios_eliminar.php",
        type: "POST",
        data: { id_usuario: id },
        dataType: "json",
        success: function(res) {
