...
$(document).ready(function() {
    ...
    const reporteTitulo = "REPORTE DE IVA";

    // Inicializar DataTable
    var table = $('#tblIva').DataTable({
        language: { 
    ...
    $("#formIva").on("submit", function(e) {
        ...
        const id = $("#id_iva").val();
        const urlDestino = (id === "") 
                ? "../../modules/iva/iva_guardar.php" 
                : "../../modules/iva/iva_actualizar.php";
    ...
    $('#tblIva').on('click', '.btnEditarIva', function() {
        const d = $(this).data();
        $("#formIva")[0].reset();
        $("#id_iva").val(d.id_iva);
        $("#nombre").val(d.nombre);
        ...
        $("#estado").val(d.estado);
        $("#modalTitulo").html('<i class="bi bi-pencil-square me-2" style="color: var(--color-yellow);"></i>Actualizar Iva: ' + d.nombre);
        $("#modalIva").modal("show");
    });
    
    // Limpiar modal al cerrar
    $('#modalIva').on('hidden.bs.modal', function() {
        $("#formIva")[0].reset();
        $("#id_iva").val("");
    });
    
    // Limpiar modal de eliminación al cerrar
    $('#modalEliminarIva').on('hidden.bs.modal', function() {
        $("#ivaEliminarId").val("");
    });
});
...
function prepararNuevo() {
    $("#formIva")[0].reset();
    $("#id_iva").val("");
    $("#modalTitulo").html('<i class="bi bi-percent" style="color: var(--color-yellow);"></i>Registrar Nuevo IVA');
}

function eliminarIva(btn) {
    // Obtener datos del botón
    const datos = $(btn).data();
    
    // Cargar datos en el modal de confirmación
    $("#ivaEliminarId").val(datos.id_iva);
    $("#eliminarNombreIva").text(datos.nombre);
    $("#eliminarPorcentaje").text(datos.porcentaje + '%');
    
    // Mostrar el modal
    $("#modalEliminarIva").modal("show");
}

function confirmarEliminarIva() {
    const id = $("#ivaEliminarId").val();
    
    if (!id) {
        showNotification('Error: ID de IVA no encontrado', 'error');
        return;
    }
    ...
    if (typeof alertify !== 'undefined') {
        alertify.confirm(
            ...
            `<div style="text-align: center;">
                ...
                <p class="text-muted">Esta acción eliminará permanentemente el IVA y no podrá ser recuperado.</p>
            </div>`,
            function() {
                ejecutarEliminacionIva(id);
            },
            function() {
        ...
        if (confirm('¿Está completamente seguro de eliminar este IVA? Esta acción no se puede deshacer.')) {
            ejecutarEliminacionIva(id);
        }
    }
}

function ejecutarEliminacionIva(id) {
    // Mostrar loading
    showNotification('Eliminando IVA...', 'info');
    
    $.ajax({
        url: "../../modules/iva/iva_eliminar.php",
        type: "POST",
        data: { id_iva: id },
        dataType: "json",
        success: function(res) {
