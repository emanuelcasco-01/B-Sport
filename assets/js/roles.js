...
$(document).ready(function() {
    ...
    const reporteTitulo = "REPORTE DE ROLES";
    ...
    $('#tblRoles').on('click', '.btnEditarRol', function() {
        const d = $(this).data();
        
        $("#id_rol").val(d.id);
        $("#nombre_rol").val(d.nombre);
        $("#descripcion_rol").val(d.desc);
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(res) {
                if (res.status && res.permisos) {
                    res.permisos.forEach(p_id => {
                        $("#p_" + p_id).prop('checked', true);
                    });
                }
            },
            error: function() {
                showNotification('No se pudieron cargar los permisos del rol', 'warning');
            }
        });
    });
