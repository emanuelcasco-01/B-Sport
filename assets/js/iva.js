...
function prepararNuevo() {
    $("#formIva")[0].reset();
    $("#id_iva").val("");
    $("#modalTitulo").html('<i class="bi bi-percent" style="color: var(--color-yellow);"></i>Registrar Nueva Categoría');
}

function eliminarIva(id, user) {
    if (typeof alertify === 'undefined') {
    ...
    alertify.confirm(
        ...
        `¿Está seguro de que desea eliminar el Iva <strong>${user}</strong>?`,
        function() {
            ejecutarEliminacion(id);
        },
        function() {
