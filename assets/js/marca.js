...
$(document).ready(function() {
    ...
    const reporteTitulo = "LISTADO DE MARCAS";
    ...
    $("#formMarca").on("submit", function(e) {
        ...
        const urlDestino = (id === "") 
                ? "../../modules/marca/marca_guardar.php" 
                : "../../modules/marca/marca_actualizar.php";
    ...
    // 3. EVENTO EDITAR
    $('#tblMarca').on('click', '.btnEditarMarca', function() {
        const d = $(this).data();
        
        $("#id_marca").val(d.id);
        $("#nombre").val(d.nombre);
        $("#estado").val(d.estado);
        $("#modalTituloMarca").html('<i class="bi bi-pencil-square me-2" style="color: var(--color-yellow);"></i>Actualizar Marca: ' + d.nombre);
    ...
    // 4. EVENTO ELIMINAR
    $('#tblMarca').on('click', '.btnEliminarMarca', function() {
        const id = $(this).data('id');
        const nombre = $(this).data('nombre');

        if (typeof alertify === 'undefined') {
            if (confirm(`¿Estás seguro de que deseas desactivar la marca: ${nombre}?`)) {
        ...
        alertify.confirm(
            ...
            `¿Estás seguro de que deseas <strong>desactivar</strong> la marca:<br><br><strong style="color: var(--color-red);">${nombre}</strong>?`,
            function() {
                ejecutarDesactivacion(id);
            },
            function() {
...
function ejecutarDesactivacion(id) {
    $.ajax({
        url: "../../modules/marca/marca_eliminar.php",
        type: "POST",
        data: { id_marca: id },
        dataType: "json",
        success: function(res) {
...
function prepararNuevaMarca() {
    ...
    $("#nombre").val("");
    $("#estado").val("1");
    $("#modalTituloMarca").html('<i class="bi bi-palette me-2" style="color: var(--color-yellow);"></i>Registrar Nueva Marca');
}
