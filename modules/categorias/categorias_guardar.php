...
if (empty($nombre) || empty($descripcion)) {
    echo json_encode(['status' => false, 'msg' => 'Nombre y Descripción son obligatorios']);
    exit;
}
...
if (mb_strlen($descripcion) > 200) {
    echo json_encode(['status' => false, 'msg' => 'La descripción es demasiado larga (máx 200)']);
    exit;
}
...
try {
    ...
    $sql_check = "SELECT id_categoria FROM categoria WHERE nombre = ? OR descripcion = ?";
    $stmt_check = $conexion->prepare($sql_check);
    $stmt_check->execute([$nombre, $descripcion]);

    if ($stmt_check->fetch()) {
        echo json_encode(['status' => false, 'msg' => 'El nombre o descripción ya existen']);
        exit;
    }

    // 5. Inserción Segura
    $sql_insert = "INSERT INTO categoria (nombre, descripcion, estado, creado_por) 
                   VALUES (?, ?, ?, ?)";
    
    $stmt_insert = $conexion->prepare($sql_insert);
    $ok = $stmt_insert->execute([$nombre, $descripcion, $estado, $creado_por]);

    if ($ok) {
        $response['status'] = true;
        $response['msg'] = 'Categoría registrada correctamente';
    } else {
