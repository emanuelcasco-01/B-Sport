...
$(document).ready(function() {
    ...
    const reporteTitulo = "LISTADO DE COLORES";
    ...
    $("#formColor").on("submit", function(e) {
        ...
        const id = $("#id_color").val();
        const nombre_color = $("#nombre_color").val().trim();
        
        if (nombre_color === "") {
            showNotification('El nombre del color es obligatorio', 'error');
            return false;
        }

        const urlDestino = (id === "") 
                ? "../../modules/color/color_guardar.php" 
                : "../../modules/color/color_actualizar.php";
    ...
    // 3. EVENTO EDITAR
    $('#tblColor').on('click', '.btnEditarColor', function() {
        const d = $(this).data();
        
        $("#id_color").val(d.id);
        $("#nombre_color").val(d.nombre_color);
        $("#estado").val(d.estado);
        $("#modalTituloColor").html('<i class="bi bi-pencil-square me-2" style="color: var(--color-yellow);"></i>Actualizar Color: ' + d.nombre_color);
    ...
    // 4. EVENTO ELIMINAR
    $('#tblColor').on('click', '.btnEliminarColor', function() {
        const id = $(this).data('id');
        const nombre_color = $(this).data('nombre_color');

        if (typeof alertify === 'undefined') {
            if (confirm(`¿Estás seguro de que deseas desactivar la talla: ${nombre_color}?`)) {
        ...
        alertify.confirm(
            ...
            `¿Estás seguro de que deseas <strong>desactivar</strong> el color:<br><br><strong style="color: var(--color-red);">${nombre_color}</strong>?`,
            function() {
                ejecutarDesactivacion(id);
            },
            function() {
...
function ejecutarDesactivacion(id) {
    $.ajax({
        url: "../../modules/color/color_eliminar.php",
        type: "POST",
        data: { id_color: id },
        dataType: "json",
        success: function(res) {
...
function prepararNuevoColor() {
    $("#formColor")[0].reset();
    $("#id_color").val("");
    $("#nombre_color").val("");
    $("#estado").val("1");
    $("#modalTituloColor").html('<i class="bi bi-palette me-2" style="color: var(--color-yellow);"></i>Registrar Nuevo Color');
}
