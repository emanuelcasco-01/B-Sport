...
$(document).ready(function() {
    ...
    const reporteTitulo = "LISTADO DE TALLAS";

    // 1. INICIALIZAR DATATABLE
    var table = $('#tblTalla').DataTable({
        language: { 
    ...
    // 2. GUARDAR O ACTUALIZAR ROL
    $("#formTalla").on("submit", function(e) {
        e.preventDefault();
        
        const id = $("#id_talla").val();
        const nombre = $("#nombre_talla").val().trim();
        const sigla = $("#sigla").val().trim();
        
        if (nombre === "") {
            showNotification('El nombre de la talla es obligatorio', 'error');
            return false;
        }

        if (sigla === "") {
            showNotification('La sigla es obligatorio', 'error');
            return false;
        }
        
        const urlDestino = (id === "") 
                ? "../../modules/talla/talla_guardar.php" 
                : "../../modules/talla/talla_actualizar.php";
    ...
    // 3. EVENTO EDITAR (Carga datos y consulta permisos)
    $('#tblTalla').on('click', '.btnEditarTalla', function() {
        const d = $(this).data();
        
        $("#id_talla").val(d.id);
        $("#nombre_talla").val(d.nombre_talla);
        $("#sigla").val(d.sigla);
        $("#estado").val(d.estado);
        $("#modalTituloRol").html('<i class="bi bi-pencil-square me-2" style="color: var(--color-yellow);"></i>Actualizar Talla: ' + d.nombre);
    ...
    $('#tblTalla').on('click', '.btnEliminarTalla', function() {
        const id = $(this).data('id');
        const nombre = $(this).data('nombre_talla');

        if (typeof alertify === 'undefined') {
            if (confirm(`¿Estás seguro de que deseas desactivar la talla: ${nombre}?`)) {
        ...
        alertify.confirm(
            ...
            `¿Estás seguro de que deseas <strong>desactivar</strong> la talla:<br><br><strong style="color: var(--color-red);">${nombre}</strong>?`,
            function() {
                ejecutarDesactivacion(id);
            },
            function() {
    ...
    $('#modalTalla').on('hidden.bs.modal', function() {
        $("#formTalla")[0].reset();
        $("#id_talla").val("");
        $(".check-permiso").prop('checked', false);
    });
...
function ejecutarDesactivacion(id) {
    $.ajax({
        url: "../../modules/talla/talla_eliminar.php",
        type: "POST",
        data: { id_talla: id },
        dataType: "json",
        success: function(res) {
...
// Función para limpiar el modal al crear nuevo
function prepararNuevaTalla() {
    $("#formTalla")[0].reset();
    $("#id_talla").val("");
    $("#nombre_talla").val("");
    $("#estado").val("1");
    $("#modalTituloTalla").html('<i class="bi bi-shield-plus me-2" style="color: var(--color-yellow);"></i>Registrar Nueva Talla');
}
