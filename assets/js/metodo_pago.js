// ============================================
// SCRIPT PARA GESTIÓN METODOS DE PAGO
// ============================================
$(document).ready(function() {
    ...
    const reporteTitulo = "REPORTE DE METODOS DE PAGO";
    ...
    $("#formMetodoPago").on("submit", function(e) {
        ...
        const id = $("#id_metodo_pago").val();
        const nombre = $("#nombre").val().trim();
        
        if (nombre === "") {
            showNotification('El nombre del metodo de pago es obligatorio', 'error');
            return false;
        }
        
        const urlDestino = (id === "") 
                ? "../../modules/metodo_pago/metodo_pago_guardar.php" 
                : "../../modules/metodo_pago/metodo_pago_actualizar.php";
    ...
    $('#tblMetodoPago').on('click', '.btnEditarMetodoPago', function() {
        const d = $(this).data();
        
        $("#id_metodo_pago").val(d.id);
        $("#nombre").val(d.nombre);
        $("#estado").val(d.estado);
        $("#modalTituloRol").html('<i class="bi bi-pencil-square me-2" style="color: var(--color-yellow);"></i>Actualizar Metodo de Pago: ' + d.nombre);
    ...
    $('#tblMetodoPago').on('click', '.btnEliminarMetodoPago', function() {
        const id = $(this).data('id');
        const nombre = $(this).data('nombre');

        if (typeof alertify === 'undefined') {
            if (confirm(`¿Estás seguro de que deseas desactivar el metodo de pago: ${nombre}?`)) {
        ...
        alertify.confirm(
            ...
            `¿Estás seguro de que deseas <strong>desactivar</strong> el metodo de pago:<br><br><strong style="color: var(--color-red);">${nombre}</strong>?`,
            function() {
                ejecutarDesactivacion(id);
            },
            function() {
...
function ejecutarDesactivacion(id) {
    $.ajax({
        url: "../../modules/metodo_pago/metodo_pago_eliminar.php",
        type: "POST",
        data: { id_metodo_pago: id },
        dataType: "json",
        success: function(res) {
            if (res.status) {
                showNotification(res.msg, 'success');
...
function prepararNuevoMetodoPago() {
    $("#formMetodoPago")[0].reset();
    $("#id_metodo_pago").val("");
    $("#estado").val("1");
    $("#modalTituloMetodoPago").html('<i class="bi bi-shield-plus me-2" style="color: var(--color-yellow);"></i>Registrar Nuevo Método de Pago');
}
